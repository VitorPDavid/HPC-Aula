...
void render(void)
{
    ...
    glTexSubImage2D( GL_TEXTURE_2D, 0, 0, 0, mTexture->width,  mTexture->height, GL_RGBA, GL_UNSIGNED_BYTE, NULL);
...
{   
    ...
    if (mFrames >= 10.0f)
    {
        ...
        sprintf(mWindowsLabel, "%lf (ms) / %lf / %f", mElapsedTime, (1000.0f / mElapsedTime), mRatio);
