...
void DestroyImage(tpImage * image)
{
 if (image!=NULL)
   {
    glDeleteBuffers(1, &image->bufferObject);
    glDeleteTextures(1, &image->textureHandle);
    if (image->bits!=NULL) free(image->bits);
    
    if (image->texels!=NULL) free(image->texels);
    free(image);
   }
...
static int ReadLine ( FILE *infile, char *s )
{
  char *news, *sptr;
  static char line[80]="";
...
tpImage * ReadPpmImage2RGBA(char * fileName)
{
   ...
   result = (result & ReadLine ( infile, s )); 
   width  = atoi(s);
   result = (result & ReadLine ( infile, s ));
   height = atoi(s);
...
/* Escreve e cria uma imagem a partir de um arquivo no formato ppm*/
int WritePpmImage2RGBA(tpImage* image, char * fileName)
{
   char            s[80]="";
   int             i, j, width, height, numBytesWritten;
   unsigned char   alpha;
   //tpImage       * image=NULL;
   FILE          * infile;
   ...
   for (j=0;j<height;j++)
     for (i=0;i<width;i++)
       {
        numBytesWritten = 0;
        numBytesWritten =  fwrite(&(image->bits[image->numComp*(i+(height-j-1)*width)+0]), 1, 1, infile) + 
                           fwrite(&(image->bits[image->numComp*(i+(height-j-1)*width)+1]), 1, 1, infile) +
                           fwrite(&(image->bits[image->numComp*(i+(height-j-1)*width)+2]), 1, 1, infile);
        ...
        alpha = image->bits[image->numComp*(i+(height-j-1)*width)+3];  
       }
...
{ 
 ...
 image->texels = (uchar4 *)malloc(width*height*sizeof(uchar4));
 memset(image->texels,0,width*height*sizeof(uchar4));
 ...
 gluScaleImage(format, image->width, image->height, GL_UNSIGNED_BYTE, image->bits, width, height, GL_UNSIGNED_BYTE, image->texels);
 
 glTexImage2D(GL_TEXTURE_2D,0,format,width,height,0,format,GL_UNSIGNED_BYTE, image->texels);
 glGenBuffers(1, &image->bufferObject);
 glBindBuffer(GL_PIXEL_UNPACK_BUFFER_ARB, image->bufferObject);
 glBufferData(GL_PIXEL_UNPACK_BUFFER_ARB, width * height * sizeof(uchar4), image->texels, GL_STREAM_COPY);
...
void DrawImage(tpImage * image,       /* Ponteiro para uma imagem.                                 */
               ...
               double sizex, double sizey,  /* Tamanho final da imagem desenhada.                        */
