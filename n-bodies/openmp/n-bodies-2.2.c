...
void particleParticle (tpParticle *particles, int nParticles, int timesteps, float dt, int threads){
...
#pragma omp parallel shared(particles, nParticles, dt)
{
        ...
        ////---------------------------------------------------
          ...
          for(int i = 0; i < nParticles; i++){
              ...
              particles[i].p.z += particles[i].v.z * dt;
...
int main (int ac, char **av){
	  int timesteps  = atoi(av[1]),
	    ...
	    fprintf(stdout, "\nP2P particle system \n");
	    fprintf(stdout, "Memory used %lu bytes  %d threads \n", nParticles * sizeof(tpParticle), threads);
	    ...
	    particleParticle(particles, nParticles, timesteps, dt, threads);
